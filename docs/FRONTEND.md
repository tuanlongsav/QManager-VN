...
- **Transfer compression** is handled server-side by lighttpd `mod_deflate` (gzip) — the installer auto-installs the module and enables it on every install (OTA included), self-validating with `lighttpd -tt`. See `docs/BACKEND.md` / `docs/reference/qmanager-independence.md`. The build doesn't need to pre-compress.
