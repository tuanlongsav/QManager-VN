...
- **Optional gzip compression (`mod_deflate`)**: Enabled *brick-safely*. `lighttpd.conf` never names `mod_deflate` directly — it ends with `include_shell "cat /usrdata/qmanager/lighttpd-deflate.conf 2>/dev/null || true"`. The installer writes that deflate snippet (`server.modules += ("mod_deflate")` + `deflate.*`) **only after `opkg install lighttpd-mod-deflate` succeeds**, and `rm -f`s it on failure. Because this lives in `install_dependencies()` (gated on `DO_PACKAGES=1`) and OTA always passes `--skip-packages`, no offline/OTA/rollback path can ever reference a module that isn't installed. Absent file → empty include → server starts uncompressed. Compresses `text/html`, `text/css`, `application/javascript`, `application/json`, `image/svg+xml`, `text/plain` above `min-compress-size` 256 B.
