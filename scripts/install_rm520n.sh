...
RCEOF
    ...
    if [ "$_opkg_ready" = "1" ]; then
        ...
        # Optional gzip compression (mod_deflate). Enabled ONLY when the module
        # installs cleanly: we then write the deflate snippet that lighttpd.conf
        # pulls in via include_shell. If the module is unavailable we delete the
        # snippet instead, so lighttpd never tries to load a missing module.
        # This block runs only on a full install (DO_PACKAGES=1) with internet;
        # OTA updates pass --skip-packages and never reach it, so an already
        # enabled compression keeps working and a never-enabled one stays off.
        DEFLATE_CONF="$QMANAGER_ROOT/lighttpd-deflate.conf"
        if "$OPKG" install lighttpd-mod-deflate >/dev/null 2>&1; then
            info "lighttpd-mod-deflate installed — enabling gzip compression"
            cat > "$DEFLATE_CONF" << 'DEFLATEEOF'
# Auto-generated by install_rm520n.sh after lighttpd-mod-deflate was confirmed
# installed. Pulled in by lighttpd.conf via include_shell. Removing this file
# (or the module) cleanly disables compression — lighttpd still starts.
...
DEFLATEEOF
            ...
            warn "lighttpd-mod-deflate not available — serving uncompressed"
            rm -f "$DEFLATE_CONF"
        fi
